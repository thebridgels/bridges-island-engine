...
| 21 | Visitor visibility within an Island; no invisible entry; no tracking beyond it | **Partly enforced** | Within-Island actorship exists in the ledger; RLS confines every record to its own Island, so cross-Island profiling has no data path (principle 17). Gaps (all not yet built): **pre-entry visitor disclosure across owned interiors** — Islands, Places, bridged spaces, protected Asset areas, and Architect spaces ("before you enter, here is what the owner can see"), not just Island-crossing; full **read-actorship of entries and reads** (capturing visitor *entries* and reads, not just writes); owner-scoped **cross-Island review of the same identified visitor**; an **owner-enabled read-only "public window" mode** (today there is no anonymous access at all — principle 3). **Visitor–Architect conversation is itself deferred to phase 2**, and when it ships it must carry the same entry-visibility/disclosure rules. The "anonymous may observe, not act" rule holds trivially now (no anonymous access and no visitor writes exist), and becomes a real design constraint the moment either ships. |
